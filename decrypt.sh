...
function main() {
   ...
   local cpt_suffixed_filenames=() base64_encrypted_basename decrypted_basename exit_code digest dirname basename;
   ...
   done < <(find "$@" -mindepth 1 -name .cpt -print0 | tac --separator=$'\0');
